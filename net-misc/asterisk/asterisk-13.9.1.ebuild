...
"
IUSE="${IUSE_VOICEMAIL_STORAGE} alsa bluetooth calendar +caps cluster curl dahdi debug doc freetds gtalk http iconv ilbc xmpp ldap libedit libressl lua mysql newt +samples odbc opus osplookup oss portaudio postgres radius selinux snmp span speex srtp static syslog vorbis vp8"
...
src_prepare() {
	default
	epatch "${FILESDIR}"/${P}-fix-configure-ac-for-pjproject.patch
	epatch "${FILESDIR}"/${P}-support-for-opus-vp8.patch
